...
func (t *Translator) DecodeOld(buf []byte) (any, error) {
	dom := t.parseOld(buf)

	res := t.deserializeOld(dom)
...
func (t *Translator) parseOld(input []byte) models.DOMPart {
	if len(input) == 0 {
		return models.DOMPart{}
	}
	
	idx, root := t.parsePartOld(0, input)
	idx++

	if root.ContentLen > 0 {
		for range root.ContentLen * 2 {
			offset, part := t.parsePartOld(idx, input)
...
func (t *Translator) parsePartOld(index int, input []byte) (int, models.DOMPart) {
	...
	case '%': // Maps
		...
		index, part.ContentLen = t.parsePartLenOld(index, input)
	...
	case '$': // Bulk strings
		...
		index, part.ValueLen = t.parsePartLenOld(index, input)
...
func (t *Translator) parsePartLenOld(index int, input []byte) (int, int) {
	var partLen int
	var lenBytes []byte
...
func (t *Translator) deserializeOld(domObj models.DOMPart) any {
	var result any
	...
	case "error":
		return t.deserializeErrorOld(domObj)
...
func (t *Translator) deserializeErrorOld(domObj models.DOMPart) error {
