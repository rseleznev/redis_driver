...
func (t Translator) EncodeOld(buf *models.SendBuf, params []any) error {
	...
	for i, v := range params {
		parts[i], err = t.buildDOMPartOld(v)
		if err != nil {
			return err
		}
	}
	...
	// Сериализуем в RESP
	buf.Buf = t.serializeDOMToRESPOld(buf.Buf, arr)
...
// string, 
// []byte, 
func (t Translator) buildDOMPartOld(input any) (models.DOMPart, error) {
...
func (t Translator) serializeDOMToRESPOld(buf []byte, input models.DOMPart) []byte {
	if input.PartType != "array" {
		panic("некорректный корневой элемент")
	}

	arrPart := t.serializeDOMPartToRESPOld(input)
	buf = append(buf, arrPart...)

	for _, v := range input.Content {
		part := t.serializeDOMPartToRESPOld(v)
		buf = append(buf, part...)
	}
...
func (t Translator) serializeDOMPartToRESPOld(input models.DOMPart) []byte {
